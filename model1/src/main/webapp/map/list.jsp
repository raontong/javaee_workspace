...
<%
    ...
    */
    ...
    // 웹브라우저인 클라이언트에게 결과 무자열을 전송해야 함..
    // 결과 정보가 현재 문자열 형태가 아닌 자바의 객체로 담겨져 있다.. 
    // ★ 하지만 웹상의 응답정보는 반드시 문자열로 변환하여ㅏ 보내야 한다!!
    // 마치 오전수업에서 JSON 내장 객체가 객체를 문자열로 바꾸거나 (JSON.stringify()), 
    // 문자열로 된 JSON을 객체리터럴로 변환하듯 (SJON.parse()), 
    // 자바에서도  JACKSON 라이브러리가 동일한 기능을 지원해준다..
	String json = mapper.writeValueAsString(data); // java에서 ---> json
    out.println(json);
 %>
