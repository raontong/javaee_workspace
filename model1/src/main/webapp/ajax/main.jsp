...
<script>
	/* 
	지금까지는 동기방식으로 서버에 요청을 시도 했고  그결과로 HTML을 가져와서 브라우저 화면에 출력함으로써
	...
	function sendAsync(){
		...
		xhttp.onload=function(){
			...
			*/
			...
			// 문자열을 해석하여 json구분 형식에 맞을 경우만, 객체 리터럴로 전환해준다!!
			...
			*/
			...
			//서버가 보내온 데이터가 다행히도, JSON 표기법을 준수 했으므로, 지금부터는 객체화 시켜서 화면에 렌더링 해보자
			// console.log("서버에서 받아온 데이터는", xhttp.responseText);
			console.log("서버rk 보낸 문자열을 파싱한 결과(객체화", memberList);
		}
