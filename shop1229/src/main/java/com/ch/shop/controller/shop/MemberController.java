...
public class MemberController {
	...
	@Autowired
	private RestTemplate restTemplate; // HTTP 요청능력 + 응답결과인 json 을 자동으로 java 객체로 매핑(마치 jackson 처럼)
	...
	public String handleNaverLogin(String code, HttpSession session) {
		...
		//  머리만들기
		// 몸은 비워두고, 헤더만 포함한 요청 엔터티 생성
		HttpEntity<String> userInfoRequest = new HttpEntity<>("", userInfoHeaders); // 몸은 비워넣고, 몸과머리합쳐 요청 보내기
		
		// RestTemplate으로 사용자 정보 요청
		//ResponseEntity<NaverUserResponse> userInfoResponse=restTemplate.exchange(client.getUserInfoUrl(), HttpMethod.GET, userInfoRequest, NaverUser.class);
		...
		log.debug("고유Name={}", naverUser.getName());
		log.debug("고유Email={}", naverUser.getEmail());
